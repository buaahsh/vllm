...
本分支 `fhb-dev-9-18` 包含 YOCO 的 `--align` 和 `--fast`。Align 的前向一致性结论限于已验证的配置与输入范围；Fast 优先性能，不保证 bitwise。上游发行版不包含本分支的开发改动。

### 单卡 Fast FP8（2026-09-15）

[测试脚本与计时说明](docs/yoco/performance/FAST_FP8_DECODE_BENCHMARK.md)：
FA4 / FP8 KV、FP8 latent 和专家、BF16 残差/router，启用 M1/M2/M4 的独立 W2 配置。
B200 上 512-token 缓存前缀 + 128 输出的 B1 吞吐约 **171 token/s**；
这是离线生成吞吐，包含剩余 prefill、采样和调度。脚本附固定输入，支持导出 Graph trace。
BF16 归约/采样与 latent Norm 融合实验保持默认关闭，训练代码不变。
