...
fig.suptitle("Mooncake toolagent 2x | 3643 requests | same B200 GPU2")
fig.text(
    0.5,
    0.01,
...
with (root / "timeline.csv").open() as f:
    events = list(csv.DictReader(f))
with (root / "planned-arrivals.csv").open() as f:
    planned = list(csv.DictReader(f))
ax.plot(
    [float(r["time_s"]) for r in planned],
    [int(r["count"]) for r in planned],
    color="black",
    linestyle=":",
    label="Planned arrivals",
)
for key, label in [("baseline", "Before"), ("candidate", "After")]:
    variant = next(r["variant"] for r in result["trace"] if r["implementation"] == key)
    for event, style in [("sent", "--"), ("completed", "-")]:
        rows = sorted(
            (r for r in events if r["variant"] == variant and r["event"] == event),
...
ax.set_xlabel("Seconds (first planned arrival / first actual send at zero)")
ax.set_ylabel("Requests")
ax.set_title("Mooncake 2x | 3643 requests | client ceiling 2048")
ax.grid(alpha=0.2)
