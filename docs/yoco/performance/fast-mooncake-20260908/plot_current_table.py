...
def main():
    ...
    parser.add_argument("--out", type=Path, default=RUN / "figures")
    args = parser.parse_args()
    args.out.mkdir(parents=True, exist_ok=True)
    current = json.loads((RUN / "TABLE_CURRENT.json").read_text())
    topologies = [("standalone", "Standalone (GPU5)"), ("pd", "1P1D (P4 / D5)")]
    caption = (
        "Mooncake toolagent 300-900s, 1x; B200; context <=81920; "
        "separate cache salts.\n"
        "Same physical GPUs, different run times: Fast Sep 8; "
        "Qwen/Align Sep 7, 2026.\n"
    ...
    for ax, (topology, title) in zip(axes, topologies):
        ...
        trace_path = RUN / "traces/toolagent-ctx81920-t300-900s-f1.jsonl"
