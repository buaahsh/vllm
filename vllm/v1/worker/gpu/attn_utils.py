...
def get_kv_sharing_fast_prefill_eligible_layers(
    ...
    if vllm_config.model_config.hf_text_config.model_type == "yoco":
        # YOCO writes the full shared KV before the compact cross block, so
        # its cache owner also consumes logits-only attention metadata.
        eligible_layers.update(
            shared_kv_cache_layers[name] for name in tuple(eligible_layers)
        )
