...
def FusedMoEFactory(
    ...
    yoco_policy: YocoMoEPolicy | None = None,
    ...
    if combined_output_transform is not None:
        if shared_experts is None:
            raise ValueError("combined_output_transform requires shared_experts")
        if shared_output_transform is not None:
            raise ValueError(
                "combined_output_transform and shared_output_transform are exclusive"
            )
    ...
    moe_config = FusedMoEConfig(
        ...
        apply_router_weight_before_w2=apply_router_weight_before_w2,
        yoco=yoco_policy if yoco_policy is not None else YocoMoEPolicy(),
    ...
    if apply_router_weight_before_w2:
        routed_experts_args = dict(routed_experts_args or {})
        routed_experts_args["apply_router_weight_before_w2"] = True
    ...
    if (
        shared_output_transform is not None
        or combined_output_transform is not None
        or reduce_shared_experts_separately
    ):
        runner_args = dict(runner_args or {})
        runner_args.update(
