...
from vllm.v1.yoco_cache import (
    truncate_yoco_remote_prefill,
    yoco_remote_prefill_token_count,
)
...
class NixlBaseConnectorScheduler:
    ...
    def on_new_request(self, request: "Request") -> None:
        ...
        if params is not None and params.get("do_remote_decode"):
            if self._use_yoco_final_prompt_block_split:
                truncate_yoco_remote_prefill(request, self.block_size)
    ...
    def _get_remote_prefill_token_count(self, num_prompt_tokens: int) -> int:
        ...
        if self._use_yoco_final_prompt_block_split:
            return yoco_remote_prefill_token_count(num_prompt_tokens, self.block_size)
