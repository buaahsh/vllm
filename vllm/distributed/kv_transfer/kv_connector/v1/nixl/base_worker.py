...
class NixlBaseConnectorWorker:
    ...
    def __init__(
        ...
        self._validate_yoco_cache_aliases = (
    ...
    def register_kv_caches(self, kv_caches: dict[str, torch.Tensor]):
        ...
        if self._validate_yoco_cache_aliases:
            known_addresses = {
                cache.data_ptr()
                for name, cache in xfer_buffers.items()
                if name in self._layer_specs
            }
            for name, cache in xfer_buffers.items():
                if (
                    name not in self._layer_specs
                    and cache.data_ptr() not in known_addresses
                ):
                    raise KeyError(
                        f"No KV cache spec found for unique cache layer {name!r}"
                    )
