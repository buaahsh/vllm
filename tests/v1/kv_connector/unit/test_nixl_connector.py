...
@pytest.mark.parametrize("yoco_cache_aliases", [False, True])
def test_register_kv_caches(
    ...
    with (
        ...
        if yoco_cache_aliases:
            kv_caches["cross_shared_0"] = tensor0
            kv_caches["cross_shared_1"] = tensor2

        # Execute register_kv_caches
        connector.register_kv_caches(kv_caches)
        if yoco_cache_aliases:
            registrations = mock_wrapper_instance.get_reg_descs.call_count
            with pytest.raises(KeyError, match="unique cache layer 'unknown'"):
                connector.register_kv_caches(
                    {**kv_caches, "unknown": torch.zeros_like(tensor0)}
                )
            assert mock_wrapper_instance.get_reg_descs.call_count == registrations
