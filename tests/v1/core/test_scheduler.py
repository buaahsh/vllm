...
def test_frontend_finish_keeps_kv_metadata_and_frees_once(monkeypatch):
    scheduler = create_scheduler()
    (request,) = create_requests(num_requests=1)
    scheduler.add_request(request)
    metadata = {"transfer_id": "finished-transfer"}
    calls = []

    def connector_finished(current):
        calls.append(current.request_id)
        return False, metadata

    monkeypatch.setattr(scheduler, "_connector_finished", connector_finished)
    outputs = scheduler.finish_requests_with_kv_transfer_params(
        request.request_id, RequestStatus.FINISHED_STOPPED
    )
    assert outputs == [(request.request_id, request.client_index, metadata)]
    assert request.request_id not in scheduler.requests
    assert (
        scheduler.finish_requests_with_kv_transfer_params(
            request.request_id, RequestStatus.FINISHED_STOPPED
        )
        == []
    )
